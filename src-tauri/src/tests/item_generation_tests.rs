...
use crate::{
    ...
    page_generators::{
        item_page::{generate_items_page, Item, ItemLocation},
        item_page_v1::generate_item_changes_page,
    },
};
...
fn test_generate_items_page_created() {
    ...
    let templates_path = std::path::PathBuf::from(
        "/Applications/WikiGen.app/Contents/Resources/resources/generator_assets/templates",
    );
    ...
    let result = generate_items_page(
        "testing",
        &base_path,
        &items,
        &item_locations,
...
#[test]
// Ability Page is created and present in the mkdocs.yml file
fn test_generate_items_page_v1_created() {
    let base_path =
        std::path::PathBuf::from("/Users/akeemallen/Library/Application Support/com.wikigen.dev");
