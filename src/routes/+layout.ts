...
export async function load({params}) {
    ...
    const appDataDirectoryExists = await exists("directory_existence", {dir: BaseDirectory.AppData});
    if (!appDataDirectoryExists) {
        await createDir("directory_existence", {dir: BaseDirectory.AppData, recursive: true});
    } 
